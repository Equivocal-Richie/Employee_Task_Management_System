...
if(isset($_GET['delete_task'])){
  ...
  $obj_admin->delete_task_info($sql,$action_id,$sent_po);
}
              ...
              <?php 
                if($user_role == 1){
                  $sql = "SELECT a.*, b.fullname 
                        FROM task_info a
                        INNER JOIN tbl_admin b ON(t_user_id = b.user_id) where ('{$date}' BETWEEN date(a.task_start_time) and date(a.task_end_time))
                        ORDER BY a.task_id DESC";
                }else{
                  $sql = "SELECT a.*, b.fullname 
                  FROM task_info a
                  INNER JOIN tbl_admin b ON(t_user_id = b.user_id)
                  WHERE a.t_user_id = $user_id and ('{$date}' BETWEEN date(a.task_start_time) and date(a.task_end_time))
                  ORDER BY a.task_id DESC";
                } 
